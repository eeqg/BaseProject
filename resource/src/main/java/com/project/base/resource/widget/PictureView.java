...
public class PictureView extends SimpleDraweeView {
	...
	public static void loadPicture(PictureView pictureView,
		...
		if (viewWidth <= 0) {
			viewWidth = BasicApp.SCREEN_WIDTH / 2;
		}
		if (viewHeight <= 0) {
			if (layoutParams.height == ViewGroup.LayoutParams.MATCH_PARENT) {
				viewHeight = BasicApp.SCREEN_HEIGHT / 2;
		...
		ResizeOptions resizeOptions = new ResizeOptions(
				viewWidth <= 0 ? BasicApp.SCREEN_WIDTH / 2 : viewWidth,
				viewHeight <= 0 ? BasicApp.SCREEN_HEIGHT / 2 : viewHeight
		);
